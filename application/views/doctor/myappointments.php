      ...
      <div class="box">
        <div class="box-header with-border">
          ...
          <div class="box-tools pull-right">
			<a href="<?= base_url('doctordashboard/previousappointments');?>" class="btn btn-primary">Appointment History</a>
		  </div>
        </div>
        <div class="box-body">
			 <?php
					...
					<tbody>
						<?php
							if(count($appointments))
							{
								...
								{
									$starttime = explode(" ",$x['appointment_time'])[0];
								 	$cur_time = date('H:i:s');
									$class='';
									 if(date('Y-m-d')==$x['appointment_date'])
									{
										$info = (explode(":",$cur_time));
										$sec1= $info[0]*3600+$info[1]*60+$info[2];
										//echo $sec1;
										//echo "<br>";
										$info = (explode(":",$starttime));
										$sec2= $info[0]*3600+$info[1]*60;
										//echo $sec2;
										if($sec1>$sec2)
										{
											$class ="hidden";
									...
									<tr>
										...
										<?php
											if($x['appointment_payment']=='Transaction Successful')
											{
												?>
													<td>
													<a href="<?= base_url('doctordashboard/reschedule/'.$x['appointment_id']);?>" class="btn btn-warning btn-block <?= $class; ?>">Reschedule Appointment</a>
													<button onClick="confirmcancel('<?= $x['appointment_id'];?>');" class="btn btn-danger btn-block <?= $class; ?>">Cancel Appointment</button>
													</td>
												<?php
											}
											else
							...
							else
							{
								?>
								<tr>
									<td colspan="7"><div class="alert alert-warning"><b>No appointments found.</b></div></td>
								</tr>
								<?php
							}
						?>
