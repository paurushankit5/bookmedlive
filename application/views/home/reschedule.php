...
<head>
  ...
  <title>Bookmediz:  Reschedule appointment with Dr. <?= $doc['user_name'];?></title>
		...
		<div class="row">
			...
			<div class="col-md-10 offset-md-1 col-lg-9 offset-lg-0">
				...
				<div class="widget dashboard-container my-adslist">
					<h3 class="widget-header">Reschedule Appointment <?= $ap['ap_id'];?></h3>
					<div class="col-sm-12">
					<div class="col-sm-6">
						...
						<input type = "hidden" readonly id = "ap_shift" name="ap_shift" >
					</div>
					<div class="col-sm-6">
						<button class="btn btn-primary" onclick="showtimeslots();">Show Time Slots</button>
					</div>
...
?>	
  ...
  <script>
  	...
  	function dateselected(id,ap_time,shift){
  		...
  		jq("#ap_shift").val(shift);
  		jq("#modalbtn").click();
  	}
  	function reschedulenow(){
  		var ap_date = jq("#datepicker").val();
  		var ap_time = jq("#ap_time").val();
  		var ap_shift = jq("#ap_shift").val();
  		var ap_id 	= "<?= $ap['ap_id'];?>";
  		var usewallet = 1;
		jq.ajax({
			type : 'POST',
			url  :  "<?= base_url('payment/reschedulenow');?>",
			data :  {
						"ap_date"  		: ap_date,
						"ap_time"       : ap_time,
						"ap_id"       	: ap_id,
