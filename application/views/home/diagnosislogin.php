...
<head>
  ...
  <title>Bookmediz: Diagnosis Login</title>
  <meta charset="utf-8">
...
<div class="col-sm-6 col-sm-offset-3" style="background-color:white;margin-top:100px;box-shadow:0px 0px 5px 5px gray;"  >
  <h2 class="text text-center text-primary" ><b>Diagnosis Login </b>	</h2>
  <?php
  ...
  ?>
  <form class="form-horizontal" method="post" action="<?=base_url('Diagnosislogin/verifylogin');?>">
    <div class="form-group">
      <label class="control-label col-sm-2" for="path_email">Email:</label>
      <div class="col-sm-10">
        <input type="email" class="form-control" id="path_email" name="path_email" placeholder="Enter email">
      </div>
    </div>
    <div class="form-group">
      <label class="control-label col-sm-2" for="path_password">Password:</label>
      <div class="col-sm-10">          
        <input type="password" class="form-control" id="path_password" name="path_password" placeholder="Enter password">
      </div>
    </div>
   <!--<div class="form-group"> 
    ...
    <div class="form-group">        
      <div class="col-sm-offset-2 col-sm-10">
		...
		<a href="<?= base_url('pathregistration');?>">New User?</a>
...
<script>
	...
	{
		var path_email	=	$('#forgotemail').val();
		if(path_email	=='')
		{
		...
		else
		{
			$.ajax({
					type		:	'POST',
					url			:	'<?= base_url('pathlogin/forgotpassword');?>',
					data		:	'path_email='+path_email,
