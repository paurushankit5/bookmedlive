<?php
	...
	$searched_name			=	$array['searched_name'];
	$limit					=	$array['limit'];
...
<div class="container-fluid" style="margin-top:-100px;">
			<div class="row" style="min-height:400px;">
				...
				<div class="col-sm-3">
					...
					<div class="list-group">
						...
						<span href="#" class="list-group-item " style="padding:10px 0px;">			 
						&nbsp; 		 
							...
							<!--<div class="col-sm-10" >
								<div class="panel panel-success" style="border-radius:0px;">
									...
									<div class="panel-body" style="font-size:14px;">
										<div class="col-sm-3" style="verical-align:center;">
											<center>
										...
										<div class="col-sm-9">
											<?php
												...
												{
													?>
														<hr> Clinic - <a href='<?= base_url('clinic/details/'.$x['doctor_clinic_id']);?>'><?= $x['clinic_name']; ?></a>
													<?php
												}										 
												if($x['qualification']!='')
												{
												...
												if($x['doctor_city']!='' || $x['clinic_city']!='')
												{
													?>
														...
														<?php
															if($x['doctor_clinic_id']!='0')
															{
																echo $x['clinic_address'].' '.$x['clinic_city'];
															}
															else{
																echo $x['doctor_address'].' '.$x['doctor_city'];
															}
										...
										<div class="col-sm-6" >
											<table class="table table-responsive">
												...
												{
													?>
													<tr><td colspan="2"><i class="fa fa-medkit"></i> <?= $x['doctor_experience'];?> years of experience</td></tr>
													<?php
												}
												if($x['doctor_fee']!=0)
												{
													?>
														...
														<tr><td><label>Consultation Fees:</label></td><td>  <?= $x['doctor_fee'];?></td></tr>
													<?php
												}
											...
											if(trim($days)!='')
												{
													?>
													<tr><td> <label>  Available Days</label> </td><td>
													  <?= $days; ?></td></tr>
													<?php
												}
											?>
										...
										<div class='col-sm-6'>
											...
											<table class="table table-responsive">
												<tr><td>Timings</td></tr>
												<tr><td>
													<?= date('h:i:A',strtotime($x['doctor_morning_start_time']));?> - <?= date('h:i:A',strtotime($x['doctor_morning_end_time']));?>
													
												</td></tr>
												<tr><td>
													<?= date('h:i:A',strtotime($x['doctor_evening_start_time']));?> - <?= date('h:i:A',strtotime($x['doctor_evening_end_time']));?>
												</td></tr>
											</table> 
									...
									<div class="panel-footer" style="padding:0px;">
										...
										<div class="col-sm-4" style="margin:0px;">
											<?php
												...
												{
												?>
												<button class="btn btn-success btn-block" onClick="openloginmodal('<?= base_url('patientdashboard/makeanappointment/'.$x['doctor_id']);?>');">Fix an Appointment</button>
		...
		function applyfilter(){
			...
			$.ajax({
				...
				url		:	'<?= base_url('search/ajax_find_doctor_by_name_filter');?>',
				success	:	function(data){
							data	=	data.trim();
							if(data=='0')
							{
								$('#mainarea').html('<div class="alert alert-danger">No results.</div>');							
							}
							else{
								$('#mainarea').html(data);
								$('#loadmorefilter').show();
		...
		function loadmore(){
			...
			$.ajax({
				...
				url		:	'<?= base_url('search/ajax_find_doctor_by_name');?>',
				success	:	function(data){
							data	=	data.trim();
							if(data=='0')
							{
...
<button type="button" id="qbtn" class="hidden" data-toggle="modal" data-target="#questionmodal"> </button>
